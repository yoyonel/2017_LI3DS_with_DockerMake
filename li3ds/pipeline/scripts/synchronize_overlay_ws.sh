...
diff_with_project=$(diff -arq ${ROS_OVERLAY_WS} $PROJECT_OVERLAY_WS)
if [ $? -ne 0 ]; then
	echo_i "Difference between ${GREEN}${ROS_OVERLAY_WS}${CYAN} and ${GREEN}${PROJECT_OVERLAY_WS}${CYAN} detected !"

	# clean overlay_ws directory
	rm -rf ${ROS_OVERLAY_WS}/*

	echo_i "Copying overlay_ws for (hard) synchronization ..."
	cp -r /root/project/overlay_ws ~/.
fi