...
int main(void)
{
    ...
    printf("Has a checksum: %d - %d\n", nmea_get_checksum(sentence)>0, nmea_get_checksum(sentence));
    printf("Validate sentence: %d\n", nmea_validate(sentence, strlen(sentence), nmea_get_checksum(sentence)>0));
    // Pointer to struct containing the parsed data. Should be freed manually.
    nmea_s *data;

    // Parse...
    data = nmea_parse(sentence, strlen(sentence), nmea_get_checksum(sentence)>0);
